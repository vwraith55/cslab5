...
zip_paths = [
    ...
    'trump_tweet_data_archive/condensed_2011.json.zip',
    'trump_tweet_data_archive/condensed_2012.json.zip',
    'trump_tweet_data_archive/condensed_2013.json.zip',
    'trump_tweet_data_archive/condensed_2014.json.zip',
...
keywords = ['obama', 'trump', 'mexico', 'russia', 'fake news', 'media', 'hillary', 'china']
...
for keyword, count in counts.items():
    percentages[keyword] = (count / total) * 100

print(percentages)

print(f"| {'phrase':>17} | {'percent of tweets':>17} |")
print(f"| {'-'*17} | {'-'*17} |")
for keyword, count in sorted(counts.items()):
    percent = (count / len(tweets)) * 100
    print(f"| {keyword:>17} | {percent:05.2f}             |")

import matplotlib.pyplot as plt

labels = sorted(counts.keys())
percentages = [(counts[k] / len(tweets)) * 100 for k in labels]

plt.bar(labels, percentages)
plt.xlabel('Phrase')
plt.ylabel('Percent of Tweets')
plt.title('Trump Tweet Keywords')
plt.xticks(rotation=45, ha='right')
plt.tight_layout()
plt.savefig('tweet_plot.png')
plt.show()

# import zipfile
# import json

# zip_path = 'condensed_2009.json.zip'
# tweets = []
# with zipfile.ZipFile(zip_path, "r") as z:
    # for file_name in z.namelist():
        # with z.open(file_name) as f:
...
# keywords = ['obama', 'trump', 'mexico', 'russia', 'fake news']
# counts = {k: 0 for k in keywords}
...
with open('README.md', 'w') as f:
    f.write("# Trump Tweet Analysis\n\n")
    
    f.write("This table and chart show the percentage of tweets containing key phrases from Trump's tweets (2009-2018).\n\n")
    
    # table
    f.write(f"| {'phrase':>17} | {'percent of tweets':>17} |\n")
    f.write(f"| {'-'*17} | {'-'*17} |\n")
    for keyword, count in sorted(counts.items()):
        percent = (count / len(tweets)) * 100
        f.write(f"| {keyword:>17} | {percent:05.2f}             |\n")
